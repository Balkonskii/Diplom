...
//				startActivity(myIntent);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (item.getItemId() != R.id.logout) {
			...
			if (count == 0) {
				new MyAsyncTask(getApplicationContext()).execute("Нет ни одного профиля");							
				return true;
			} else if (selectedChildrenId == -1) {
				new MyAsyncTask(getApplicationContext()).execute("Ничего не выбрано");				
				return true;
			}
		}
		...
		case R.id.notifications:
			DataManager.getCurrentState().notificationsClicked(getBaseContext());
			return true;
		case R.id.logout:
